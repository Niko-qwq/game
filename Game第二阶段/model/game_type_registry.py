...
class GameTypeRegistry:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance._ai_strategies = {}  # {game_type: {difficulty: AIStrategy子类}}
    ...
    def register_ai_strategy(self, game_type, difficulty, strategy_class):
        """
        注册AI策略类
        :param game_type: 游戏类型标识
        :param difficulty: 难度等级（"easy", "normal", "hard"）
        :param strategy_class: AI策略类，必须是AIStrategy的子类
        """
        if game_type not in self._ai_strategies:
            self._ai_strategies[game_type] = {}
        self._ai_strategies[game_type][difficulty] = strategy_class
    
    def unregister_ai_strategy(self, game_type, difficulty=None):
        """
        注销AI策略类
        :param game_type: 游戏类型标识
        :param difficulty: 难度等级，如果为None则注销该游戏类型的所有AI策略
        """
        if game_type in self._ai_strategies:
            if difficulty:
                if difficulty in self._ai_strategies[game_type]:
                    del self._ai_strategies[game_type][difficulty]
            else:
                del self._ai_strategies[game_type]
    
    def get_ai_strategy(self, game_type, difficulty):
        """
        获取AI策略类
        :param game_type: 游戏类型标识
        ...
        game_ai_strategies = self._ai_strategies[game_type]
        if difficulty not in game_ai_strategies:
            raise ValueError(f"游戏类型 {game_type} 不支持难度 {difficulty}")
        
        return game_ai_strategies[difficulty]
    ...
    def get_supported_difficulties(self, game_type):
        ...
        if game_type not in self._ai_strategies:
            return []
        return list(self._ai_strategies[game_type].keys())
