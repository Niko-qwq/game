...
from controller.command import MoveCommand, UndoCommand, ResetCommand
...
class GameController(Subject):
    ...
    def start_game(self, game_type="gomoku", board_size=15):
        ...
        if not self.gui_view:
            ...
            # 将GUIView作为观察者附加到GameController
            self.attach(self.gui_view)
            # 运行GUI主循环
            self.gui_view.run()
        else:
            # 通知观察者更新视图
            self.notify(game_model=self.game_model)
    ...
    def handle_resign(self):
        ...
        if self.game_model and not self.game_model.is_game_over():
            ...
            # 调用模型的pass_move方法来设置游戏结束
            self.game_model.pass_move()
            
            # 设置获胜者
            self.game_model.winner = opponent
