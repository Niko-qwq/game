...
class ConfigFrame(ttk.LabelFrame):
    ...
    def setup_widgets(self):
        ...
        self.game_type_combo = ttk.Combobox(
            self, textvariable=self.game_type_var, values=[
                "五子棋", "围棋"], state="readonly", width=10)
    ...
    def start_game(self):
        ...
        # 获取游戏类型
        game_type_text = self.game_type_var.get()
        if game_type_text == "五子棋":
            game_type = "gomoku"
        elif game_type_text == "围棋":
            game_type = "go"
