...
class MementoFactory:
    ...
    def create_memento(game_type, board, current_player, **kwargs):
        ...
        if memento_class:
            # 检查备忘录类的__init__方法接受哪些参数
            init_signature = inspect.signature(memento_class.__init__)
            init_params = list(init_signature.parameters.keys())
            
            # 过滤出__init__方法实际接受的关键字参数
            # 跳过前4个参数：self, board, current_player, game_type
            valid_kwargs = {}
            for key, value in kwargs.items():
                if key in init_params:
                    valid_kwargs[key] = value
            
            # 使用注册的备忘录类，只传递它能接受的参数
            return memento_class(board, current_player, game_type, **valid_kwargs)
        else:
